    ...
    messages.forEach(message => {
      ...
      switch (messageType) {
        case APP_CONSTANTS.PROCESSES.RUN_PRE_REFRESH_CHECKS:
          console.log(
            `Run prerefreshcheck message received from the background - opening myactivity page`
          );
...
function runPreRefreshCycleChecks(activeUserInPopup) {
  location.hash = APP_CONSTANTS.EXTENSION_URL_HASH;
...
function extractAndSaveOtherUsers() {
  ...
  let otherUserIds: Array<string> = userLoginDivElemsArr.map(userLoginDivElem =>
    (userLoginDivElem.textContent || '').trim()
  );
  otherUserIds = otherUserIds.map(userId =>
    userId.includes('default') ? userId.replace(' (default)', '') : userId
  );
...
function getYoutubeWatchHistorySetting(): boolean {
  const youtubeWatchHistoryElementParent: Element = document.querySelector(
    'div[data-aid^="youtubeWatch"]'
  );
  const youtubeWatchHistoryElement: any = youtubeWatchHistoryElementParent.firstChild;
  const youtubeHistoryControlStatus: boolean =
    youtubeWatchHistoryElement.getAttribute('aria-checked') === 'true';
    ...
    sendMessageToBackgroundScript(
      {
        ...
        data: {
          message:
            'Kindly enable youtube watch history. Without which youtube watch history extension cannot work properly',
          title: ``
        }
      },
      ExtensionModule.ContentScript
...
function initiateRefreshCycle(activeUserInPopup = '') {
  ...
  console.log(
    `Activity controller content script : redirecting to myactivity page. Initiated by the background script...`
  );
  location.href = `${APP_CONSTANTS.MY_ACTIVITY_PAGE_URL}#${
    APP_CONSTANTS.EXTENSION_URL_HASH
  }/${shaConvertedActiveUserId}`;
}
...
function compareUsers(loggedInUser: string) {
  ...
  if (loggedInUser !== activeUserInPopup) {
    console.log(
      `Logged in User and Active User in differ. Logged In User : ${loggedInUser}, User Active in Popup : ${activeUserInPopup}`
    );
...
function logInToActiveUserAccount(activeUserInPopup: string) {
  ...
  let isActiveUserNotLoggedIn: Boolean =
    activeUserLoginDivParent.children.length > 2 ||
    Boolean(activeUserLoginDivParent.querySelector('.gb_xb'));
