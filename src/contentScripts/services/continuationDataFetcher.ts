...
class ContinuationDataService {
   ...
   */
  public async fetchNextContinuationData(
    continuationDataFetchingParam: string,
    userId: string,
    sendContinuationDataToBackgroundScript = true
  ): Promise<any> {
    let message: any = {};
    ...
    console.log(
      'Fetching continuation data request response :',
      response,
      responseString.slice(0, 500),
      '\n\n',
    ...
    if (responseData && !isEmpty(responseData)) {
      ...
      if (isNil(newlyWatchedVideos)) {
        console.log(
          'Newly watched videos missing from the response :    ' +
            newlyWatchedVideos +
            '  ' +
            newContinuationParams
        );
        message = {
  ...
  public sendContinuationDataToBackgroundScript(message: IExtensionEventMessage) {
    const activeUrl: string = window.location.href;
    const activePage: ActivePage = getActivePage(activeUrl);
    const sender = ExtensionModule.ContentScript;
