...
} from './variableScriptCommunicator';
...
export async function startDataExtractionProcess(lastSavedVideoDetails: IYoutubeVideo) {
  ...
  const initialDataScript: any = Array.from(scriptsWithNonceAttribute).filter(
    (script: any) =>
      script.innerText.startsWith('(function(){window.HISTORY_frontend_user_state')
  );

  if (initialDataScript) {
    lastSavedVideo =
      lastSavedVideoDetails && Object.keys(lastSavedVideoDetails).length
        ? lastSavedVideoDetails
        : null;
    insertInitialDataLoadingScript(initialDataScript[0]);
    console.log(
      `History extraction started...Inserting the variable access script to extract the initial history data`
    );
...
function insertInitialDataLoadingScript(initialDataScript: HTMLScriptElement) {
  ...
  const requireEndIndex: number = requiredScriptInnerText.indexOf(
    'window.HISTORY_used_products'
  );
...
async function saveInitialHistoryData(message) {
  ...
  }: INewInitialHistoryData = data;
  console.log('inside saving initial history data : ', message, lastSavedVideo);

  newlyWatchedVideos = newlyWatchedVideos.map(
    (newVideo, index) => new YoutubeVideo(newVideo, index)
  );

  if (lastSavedVideo && Object.keys(lastSavedVideo).length) {
    console.log(
      'SavingInitialHistoryData : Last saved video exists. Finding intersection point of last saved video'
    );
    ({ newlyWatchedVideos, continuationDataFetchingParam } = await getVideosToSave(
      newlyWatchedVideos,
      continuationDataFetchingParam,
      false
    ));
  ...
  if (!isEmpty(newlyWatchedVideos)) {
    console.log(
      'Sending initial history data to background script : ',
      newlyWatchedVideos
    );
    sendMessageToBackgroundScript(
      {
...
async function getVideosToSave(
  newlyWatchedVideos: IYoutubeVideo[],
  continuationDataFetchingParam,
  userId,
  recursiveCall = false
) {
  const intersectingVideoIndex: number = newlyWatchedVideos.findIndex(
    (video) => video.videoId === lastSavedVideo.videoId
  );

  if (intersectingVideoIndex === 0) {
    console.log(
      'SavingInitialHistoryData : Intersection point coincides with the first video. Do nothing'
    );
    return {
    ...
    console.log(
      'SavingInitialHistoryData : Intersection point doesn\'t exist. Fetching remaining Initial History Data'
    );
    // remaining initial history data case
    const message: any = await ContinuationDataService.fetchNextContinuationData(
      continuationDataFetchingParam,
      userId,
      false
    );
    const remainingInitialHistoryData: any = message.data;
    const { newlyWatchedVideos: newlyFetchedVideos } = remainingInitialHistoryData;
    ...
    console.log(
      'SavingInitialHistoryData : Intersection point found. Saving new data till intersection point'
    );
    // Intersection case
