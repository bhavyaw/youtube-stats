...
function handleMessageFromContentScript(event) {
  ...
  if (sender === ExtensionModule.ContentScript) {
    ...
    switch (contentType) {
      ...
      case APP_CONSTANTS.DATA_EXCHANGE_TYPE.EXTRACT_MY_ACTIVITY_PAGE_DATA:
        extractMyActivityPageData(activePage);
        break;
    }
  }
}

function extractMyActivityPageData(activePage: ActivePage) {
  ...
  if (activePage === ActivePage.activityControls) {
    ...
    userId = windowNode['WIZ_global_data']['oPEP7c'];
    const continuationDataFetchingParams = getContinuationDataFetchingParams(windowNode);

    // olduserId_0 = windowNode['HISTORY_account_email'];
...
function getContinuationDataFetchingParams(windowNode) {
  let continuationDataFetchingParams = {};
  const wizGlobalData = get(windowNode, 'WIZ_global_data');

  if (!wizGlobalData) {
    reportVariableAccessError(APP_MESSAGES.ERROR_MESSAGES.dataFetchingProcessChanged);
  }

  const rpcids = get(windowNode, ['AF_dataServiceRequests', 'ds:1', 'id']);
  const fsid = get(wizGlobalData, 'FdrFJe');
  const bl = get(wizGlobalData, 'cfb2h');
  ...
  return continuationDataFetchingParams;
}
...
const APP_CONSTANTS = {
  ...
  DATA_EXCHANGE_TYPE: {
    ...
    ERROR: 'ERROR',
    EXTRACT_MY_ACTIVITY_PAGE_DATA: 'EXTRACT_MY_ACTIVITY_PAGE_DATA',
    MY_ACTIVITY_PAGE_DATA: 'MY_ACTIVITY_PAGE_DATA'
  }
};
